...
// Â§P5 B2 (2026-07-22) â€?GBufferPass empty shell.
//
...
// docs/pass-lessons-from-deferred.md Â§5.2). Real LightingPass that
// binds the GBuffer as its input lands in B5.
//
// Â§5.3 red lines we still respect:
//   - No FrameContext writeback of gbuffer RTs (would force constâ†?//     non-const and re-trigger the Â§5.5 PR-F1' FrameContext ABI
...
//     additive â€?same shape as PR-F2's shadowPass field).
//
...
//   plumbing-only ShadowPass commit â†?PR-F2 shadowPass pointer
//   commit â†?PR-F3 caster state commit â†?live shadow ship later).
//
...
public:
    // Â§P5 B4 lands real MRT â€?RT0 albedo RGBA8, RT1 normal RGBA8,
    ...
    // Â§P5 B4a (2026-07-22) â€?fix B2 known-bug `&& false`. Now reads:
    // "FBO handle index is valid (â‰?UINT16_MAX)". Default-constructed
    ...
    // Stub accessors â€?return invalid handles / identity until B4
    ...
    // Â§P5 B4a (2026-07-22) â€?depth attachment accessor. B5 LightingPass
    // doesn't sample depth (samples albedo/normal/motion only) but
    // future B7+ multi-light chain / DebugOverlay GBuffer visualization
    // (cutsheet Â§6.2) may need linearized depth. Mirror the 3 color
    // accessors' shape â€?BGFX_INVALID_HANDLE until B4 ensure wires the
    // 4-attach MRT.
    ...
    // Â§P5 B4a (2026-07-22) â€?build stamp pointer (mirror
    // ShadowMapResources.h:55 `const char* _buildStamp` shape).
    // Comparison via pointer equality (NOT string compare) â€?callers
    ...
    // Â§P5 B4b (2026-07-22) â€?lazy Phoskia GBuffer VS/FS acquire.
    ...
    // Â§P5 B4c (2026-07-22) â€?host pushes previous-frame view +
    ...
    // (PÃ—V same-order as `setViewTransform` + `viewProjectionMatrix`
    // builtin ordering â€?mirror `docs/pass-lessons-from-shadow.md`
    // Â§3.1 warning). The host just hands the raw pieces.
    //
    // Identity default: first frame, prev is identity (set by Impl
    // default-init). B4c documents garbage motion on frame 0 as
    // acceptable â€?B7+ TAA consumer must tolerate one-frame seed
...
private:
    // Â§P5 B4a (2026-07-22) â€?add depth RT handle + build stamp pointer.
    ...
    // _allocatedW/H in ensure() â€?must NOT reuse the request fields
    ...
    // Â§P5 B4a (2026-07-22) â€?internal ensure (mirror
    ...
    // Â§P5 B4b (2026-07-22) â€?Phoskia GBuffer VS/FS program handle
    // (mirror ShadowCaster::_program).
    ayt::shader::ShaderResource _program;
    bool _acquireFailed = false;

    // Â§P5 B4c (2026-07-22) â€?previous-frame view/projection cache
