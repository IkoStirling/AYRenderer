...
//
// §P5.5 B (2026-07-23) �?Bug fix #3: cache-key bump v20 �?v21
...
material Lighting {
    ...
    fragment {
        ...
        // §P5.5 C �?per-slot shadow factors. Mirror contract: lights[0]
...
TEST_CASE(b5p5_key_light_shadow_only_fill_unshadowed) {
    ...
    //
    // §P5.5 B (2026-07-23) �?per-type branching: shadowKey multiply
    ...
    // absence �?the `f1_branch` token below must NOT contain
    // `shadowKey`.
    const std::string src = mirrorLightingPhoskiaSourceB5p5();
    ...
    // the lights[0] keyContrib branch �?must NOT appear. The whole-
...
// these declarations in BOTH places (cutsheet §P5.5 C :330
...
// §P5.5 C (2026-07-23) �?K3 invariant pin: perLightShadowCount.x
// == 0 �?FS collapses to the legacy key-only shadow multiply
